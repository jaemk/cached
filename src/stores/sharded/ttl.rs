    ...
    K: Hash + Eq,
{
    ...
    /// panic. This holds whether or not an `on_evict` callback is configured. It is why the
    /// selection pass clones the doomed keys, and why the method requires `K: Clone`.
        ...
        K: Clone,
    {
        ...
        for shard in self.inner.shards.iter() {
            // Collect under the write lock, fire callbacks after releasing it.
            //
            // Two passes, matching the LRU-backed sharded stores. The first runs `keep` (user
            // code) and only selects; the second removes and runs nothing that can panic. A
            // single `extract_if` pass removes eagerly *while* `keep` runs, so a panicking
            // predicate dropped every already-yielded entry during unwind: gone from the cache,
            // never handed to `on_evict`, never counted. The no-callback path used to take a
            // `before - guard.len()` delta after an in-place `HashMap::retain`, which the same
            // unwind skipped entirely; it now shares this structure so both paths remove,
            // count, and (where configured) notify exactly the same entries.
            let removed: Vec<(K, TimedEntry<V>)> = {
                let mut guard = shard.lock.write();
                let doomed: Vec<K> = guard
                    .iter()
                    .filter_map(|(k, entry)| {
                        if expired_at(entry, now) || !keep(k, &entry.value) {
                            Some(k.clone())
                        } else {
                            None
                        }
    ...
    H: ShardHasher<K>,
{
    ...
    fn cache_set(&self, k: K, v: V) -> Result<Option<V>, Self::Error> {
        ...
        // `HashMap::insert` and the single-owner `TtlCache`. This used to branch on
        ...
        let old: Option<(K, TimedEntry<V>, bool)> = {
            ...
            match guard.get_mut(&k) {
                Some(slot) => {
                    let e = std::mem::replace(slot, new_entry);
