    ...
    if !companions && !args.in_impl && args.companions_vis.is_some() {
        return syn::Error::new(
            ...
            "`companions_vis` has no effect with `companions = false`: the only companion it \
             ...
             emit it. Remove one of the two. (Without `in_impl = true`, \
             `#[concurrent_cached]` emits the uncached `{fn}_no_cache` origin as a \
             function-local `fn` inside the cached function, not as a sibling item, so there \
             is nothing else to make visible.)",
        )
        .to_compile_error()
        .into();
    }
