                ...
                None => {
                    if time.is_none() {
                        ...
                        match time_refresh {
                            Some(time_refresh) => {
                                if asyncness.is_some() {
                                    quote! { cached::AsyncRedisCache::new(#cache_prefix, ::cached::web_time::Duration::from_secs(#time)).set_refresh(#time_refresh).build().await.expect("error constructing AsyncRedisCache in #[io_cached] macro") }
                                } else {
                                    quote! {
                                        cached::RedisCache::new(#cache_prefix, ::cached::web_time::Duration::from_secs(#time)).set_refresh(#time_refresh).build().expect("error constructing RedisCache in #[io_cached] macro")
                                    }
                                }
                            }
                            None => {
                                if asyncness.is_some() {
                                    quote! { cached::AsyncRedisCache::new(#cache_prefix, ::cached::web_time::Duration::from_secs(#time)).build().await.expect("error constructing AsyncRedisCache in #[io_cached] macro") }
                                } else {
                                    quote! {
                                        cached::RedisCache::new(#cache_prefix, ::cached::web_time::Duration::from_secs(#time)).build().expect("error constructing RedisCache in #[io_cached] macro")
                                    }
                                }
                            }
                        }
            ...
            let cache_create = match cache_create {
                ...
                None => {
                    ...
                    let create = match time {
                        ...
                        Some(time) => {
                            quote! {
                                (#create).set_lifespan(::cached::web_time::Duration::from_secs(#time))
                            }
                        }
                    };
