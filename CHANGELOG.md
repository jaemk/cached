...
- Redis values are now serialized with MessagePack (`rmp-serde`) instead of JSON; the `redis_store` feature pulls `rmp-serde` instead of `serde_json`. Existing Redis entries are recomputed on miss (the on-the-wire value format is now MessagePack). `RedisCacheError`'s serialize/deserialize variants carry `rmp_serde::encode::Error` / `rmp_serde::decode::Error` instead of `serde_json::Error`.
...
- `RedisCache::connection_string()` / `AsyncRedisCache::connection_string()` now return the redacted connection string (credentials masked). Use the new `connection_string_unredacted()` for the raw URL.
